...
describe('check', () => {
  ...
  test('address', async () => {
    ...
    {
      const { stdout, stderr, code } = await cli('check address --candidate foo');
      expect(code).toBe(0);
      expect(stdout.trimRight()).toBe('false');
      expect(stderr).toBe('');
    }
  ...
  test('work', async () => {
    expect.assertions(6);
    {
      const { stdout, stderr, code } = await cli('check work --candidate b2ff948c874e7d62');
      expect(code).toBe(0);
      expect(stdout.trimRight()).toBe('true');
      expect(stderr).toBe('');
    }
  ...
  test('signature', async () => {
    ...
    {
      const { stdout, stderr, code } = await cli('check signature --candidate foo');
      expect(code).toBe(0);
      expect(stdout.trimRight()).toBe('false');
      expect(stderr).toBe('');
    }
...
describe('convert', () => {
  test('amount', async () => {
    expect.assertions(3);
    const { stdout, stderr, code } = await cli('convert amount --input 1 --from Nano --to raw');
    expect(code).toBe(0);
