...
describe('secret keys', () => {
  ...
  test('throws with invalid seeds', () => {
    ...
    for (let invalidSeed of INVALID_SEEDS) {
      expect(() => nano.deriveSecretKey(invalidSeed, 0)).toThrowError('Seed is not valid');
    }
  });
  ...
  test('throws with invalid indexes', () => {
    ...
    for (let invalidIndex of INVALID_INDEXES) {
      expect(() => nano.deriveSecretKey(RANDOM_VALID_KEY.seed, invalidIndex)).toThrowError(
        'Index is not valid'
      );
    }
  });
...
describe('addresses', () => {
  ...
  test('throws with invalid public keys', () => {
    ...
    for (let invalidPublicKey of INVALID_PUBLIC_KEYS) {
      expect(() => nano.deriveAddress(invalidPublicKey)).toThrowError('Public key is not valid');
    }
  });
});
