...
describe('send', () => {
  ...
  test('throws with invalid previous', () => {
    ...
    for (let invalidHash of INVALID_HASHES) {
      expect(() =>
        nano.hashSendBlock(invalidHash, VALID_SEND_BLOCK.destination, VALID_SEND_BLOCK.balance)
  ...
  test('throws with invalid destination', () => {
    ...
    for (let invalidAddress of INVALID_ADDRESSES) {
      expect(() =>
        nano.hashSendBlock(VALID_SEND_BLOCK.previous, invalidAddress, VALID_SEND_BLOCK.balance)
  ...
  test('throws with invalid balance', () => {
    ...
    for (let invalidAmount of INVALID_AMOUNTS) {
      expect(() =>
        nano.hashSendBlock(VALID_SEND_BLOCK.previous, VALID_SEND_BLOCK.destination, invalidAmount)
...
describe('open', () => {
  ...
  test('throws with invalid source', () => {
    ...
    for (let invalidHash of INVALID_HASHES) {
      expect(() =>
        nano.hashOpenBlock(invalidHash, VALID_OPEN_BLOCK.representative, VALID_OPEN_BLOCK.account)
  ...
  test('throws with invalid representative', () => {
    ...
    for (let invalidAddress of INVALID_ADDRESSES) {
      expect(() =>
        nano.hashOpenBlock(VALID_OPEN_BLOCK.source, invalidAddress, VALID_OPEN_BLOCK.account)
  ...
  test('throws with invalid account', () => {
    ...
    for (let invalidAddress of INVALID_ADDRESSES) {
      expect(() =>
        nano.hashOpenBlock(VALID_OPEN_BLOCK.source, VALID_OPEN_BLOCK.representative, invalidAddress)
...
describe('change', () => {
  test('creates correct change hash', () => {
    expect(
      nano.hashChangeBlock(VALID_CHANGE_BLOCK.previous, VALID_CHANGE_BLOCK.representative)
  ...
  test('throws with invalid representative', () => {
    expect.assertions(INVALID_ADDRESSES.length);
    for (let invalidAddress of INVALID_ADDRESSES) {
      expect(() => nano.hashChangeBlock(VALID_CHANGE_BLOCK.previous, invalidAddress)).toThrowError(
...
describe('receive', () => {
  test('creates correct receive hash', () => {
    expect(nano.hashReceiveBlock(VALID_RECEIVE_BLOCK.previous, VALID_RECEIVE_BLOCK.source)).toBe(
      VALID_RECEIVE_BLOCK.hash
    );
  });
  ...
  test('throws with invalid previous', () => {
    expect.assertions(INVALID_HASHES.length);
    for (let invalidHash of INVALID_HASHES) {
      expect(() => nano.hashReceiveBlock(invalidHash, VALID_RECEIVE_BLOCK.source)).toThrowError(
        'Previous is not valid'
      );
    }
  });
  ...
  test('throws with invalid source', () => {
    expect.assertions(INVALID_HASHES.length);
    for (let invalidHash of INVALID_HASHES) {
      expect(() => nano.hashReceiveBlock(VALID_RECEIVE_BLOCK.previous, invalidHash)).toThrowError(
        'Source is not valid'
      );
    }
  });
